...
class PeerServer(threading.Thread):
    ...
    def start_tcp_server(self, port):
        self.tcpServerSocket.bind(('localhost', port))
        self.tcpServerSocket.listen(5)

    def send_udp_message(self, message, destination_ip, destination_port):
        self.udpClientSocket.sendto(message.encode(), (destination_ip, destination_port))
    ...
    def join_chat_room(self, room_name, member_username):
        if room_name in self.chat_rooms:
            chat_room = self.chat_rooms[room_name]
            chat_room.add_member(member_username)
            print(f"You've joined chat room '{room_name}'.")
        else:
            print(f"Chat room '{room_name}' doesn't exist.")

    # Method to leave a chat room
    def leave_chat_room(self, room_name, member_username):
        if room_name in self.chat_rooms:
            chat_room = self.chat_rooms[room_name]
            chat_room.remove_member(member_username)
            print(f"You've left chat room '{room_name}'.")
        else:
            print(f"Chat room '{room_name}' doesn't exist.")
    # main method of the peer server thread
    def simulate_create_rooms_load(self, num_rooms):
    ...
    def run(self):
        ...
        self.simulate_create_rooms_load(100) 
                ...
                for s in readable:
                    ...
                    # is used to communicate with a connected peer, then enters here
                        ...
                        # connected user suddenly ended the chat(an error occurred)
                            ...
                            print("Press enter to quit the chat: ")
                        if len(messageReceived) > 11 and messageReceived[:12] == "CREATE_ROOM":
                            room_name = messageReceived.split()[1]
                            self.create_chat_room(room_name)
                        elif len(messageReceived) > 9 and messageReceived[:10] == "JOIN_ROOM":
                            room_name = messageReceived.split()[1]
                            self.join_chat_room(room_name, self.username)
                        elif len(messageReceived) > 10 and messageReceived[:11] == "LEAVE_ROOM":
                            room_name = messageReceived.split()[1]
                            self.leave_chat_room(room_name, self.username)
...
class PeerClient(threading.Thread):
    ...
    def start_udp_client(self):
        # Configure UDP client to send real-time messages
        pass
