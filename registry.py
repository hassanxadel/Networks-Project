...
class ClientThread(threading.Thread):
    ...
    def handle_room_operation(self, message):
        if message[0] == "CREATE_ROOM":
            room_name = message[1]
            if not db.is_room_exist(room_name):
                db.create_room(room_name)
                db.add_user_to_room(self.username, room_name)
                response = f"room-created {room_name}"
                logging.info("Send to " + self.ip + ":" + str(self.port) + " -> " + response)
                self.tcpClientSocket.send(response.encode())
            else:
            ...
            room_name = message[1]
            if db.is_room_exist(room_name):
                db.add_user_to_room(self.username, room_name)
                response = f"room-joined {room_name}"
                logging.info("Send to " + self.ip + ":" + str(self.port) + " -> " + response)
                self.tcpClientSocket.send(response.encode())
            else:
            ...
            room_name = message[1]
            db.remove_user_from_room(self.username, room_name)
